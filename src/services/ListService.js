...
export class ListService {
	...
	async getListById(userRequesterId, id) {
		...
		if (list.user.toString() !== userRequesterId) {
			throw new Unauthorized(
				"You are not authorized to access this list, because youre not the owner"
			);
		}
	...
	async updateList(userRequesterId, id, data) {
		try {
			...
			if (list.user.toString() !== userRequesterId) {
				throw new Unauthorized(
					"You are not authorized to update this list, because youre not the owner"
				);
			}

			return await await List.findByIdAndUpdate(id, data, {
				new: true,
				runValidators: true,
			});
		} catch (error) {
	...
	async deleteList(userRequesterId, id) {
		try {
			...
			if (list.user.toString() !== userRequesterId) {
				throw new Unauthorized(
					"You are not authorized to delete this list, because youre not the owner"
				);
			}
